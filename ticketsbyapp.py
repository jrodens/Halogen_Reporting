...
for issue in datafromjson:
    ...
    if issue.fields.customfield_23201 is None:
        ...
        application = S(issue.fields.customfield_23201.value)
    Application.append(application)



newdata.insert(0,"Application",Application)
newdata.insert(1,"Service",Service)
...
newdata = newdata[(newdata.Application!='None')]
#newdata.to_excel('O:/TicketsbyService.xlsx')
newdata = newdata.sort_values(by='Service')
newdata = newdata.groupby(['Application','Service']).size()

color_dict = {'Accepted':'#7030A0','Blocked':'#FFC000','In Development':'#002060','Not Started':'#C00000','QA':'#548235','Rejected':'#808080'}
output = 'C:\\Users\\' + username + '\\OneDrive - TRowePrice\\Consumer Tickets by Application -'+ str(datetime.datetime.today().strftime('%Y%m%d')) +'.pdf'
pdf = PdfPages(output)
fig1 = plt.figure()
...
with PdfPages(output) as pdf:
    ...
    newdata.plot(ax=ax1,kind='bar',stacked=True, figsize=(20,10),color=[color_dict.get(x) for x in newdata.columns], title="Consumer Tickets by Application")
