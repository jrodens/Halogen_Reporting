...
#datafromjson2 = requests.get('https://jira.troweprice.com/rest/api/2/search?jql=project%20=%20NYC-RIO-Surge-Halogen%20and%20component%20=%20%22Positions%20Domain%22&maxResults=1000&fields=description', auth=(username, password)).content
...
datafromjson2 = pd.read_json('attributes.json', typ='series', orient='columns')
...
Item=[]
Status=[]
datafromjson2.to_excel('O:/json.xlsx')
#Parse the returned JSON
for index, row in datafromjson2.iterrows():
    ...
    description = datafromjson2.iloc[index,4]
    story = datafromjson2.iloc[index,3]
    if description != None:
        r1 = re.findall("((^| )([^ ]+_[^ ]+))( |$)|.','\1'",description)
        print(r1)
        Link.append(r1)
        Item.append(story)
        #Item.append(re.search("((^| )([^ ]+_[^ ]+))( |$)|.','\1'",description))

q1.insert(0, "Story", Item)
q1.insert(1,"Link", Link)
q1.replace(to_replace="(", value=" ")
q1.replace(to_replace=")", value=" ")
q1.replace(to_replace="'", value=" ")
q1.replace(to_replace="''", value=" ")
q1.replace(to_replace=",", value=" ")
q1.explode('Link')
q1.to_excel('O:/HalPri.xlsx')
