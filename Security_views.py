...
datafromjson = jira.search_issues('project = NYC-RIO-Surge-Halogen-Analysis and "Epic Link" = NYCRSA-1563', maxResults=300)
...
datafromjson2 = requests.get('https://jira.troweprice.com/rest/api/2/search?jql=project%20=%20NYC-RIO-Surge-Halogen%20and%20component%20=%20%22security%20Domain%22&maxResults=1000&fields=status', auth=(username, password)).content
...
output = 'C:\\Users\\' + username + '\\OneDrive - TRowePrice\\SecurityAttributes-'+ str(datetime.datetime.today().strftime('%Y%m%d')) +'.pdf'
#output = 'O:/TransactionAttributes.pdf'
pdf = PdfPages('O:/Securityattributes.pdf')
fig1 = plt.figure()
