...
def targetdatealert():
    ...
    for issue in priority:
        ...
        days = np.busday_count(targetdelivery.date(), today)
        ...
        for link in issue.fields.issuelinks:
            if hasattr(link, "outwardIssue"):
                ...
                if "NYCIO" in outwardIssue.key:
                    if (outwardIssue.fields.status.name not in 'QA' or outwardIssue.fields.status.name not in 'Accepted' or outwardIssue.fields.status.name not in 'Rejected'):
                ...
                if "NYCIO" in inwardIssue.key:
                    if (inwardIssue.fields.status.name not in 'QA' or inwardIssue.fields.status.name not in 'Accepted' or inwardIssue.fields.status.name not in 'Rejected'):
                        link = jira.issue(inwardIssue.key)
                        pointsOutstanding = pointsOutstanding + link.fields.customfield_10502
        if pointsOutstanding > pointsAvailable:
            message = '<messageML> !!!!Error, ticket '+currentID +' is NOT projected to meet delivery date!!! </messageML>'
