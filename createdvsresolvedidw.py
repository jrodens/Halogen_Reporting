...
while(createdate1 <= datetime.datetime.today()):
    ...
    string = 'project ="NYC-IO-Surge-Testing"  AND ("Projects Impacted" is EMPTY OR "Projects Impacted" in ("IDW 1.0 DE", "IDW Cutover", RIO-RR, "IDW 1.0 Publications", "BAU (Business as Usual)", "SV MoM")) AND status was not in (Accepted, Rejected) on '+cd1 +' AND status was in (Accepted) on ' +cd2
    string2 = 'project ="NYC-IO-Surge-Testing" AND ("Projects Impacted" is EMPTY OR "Projects Impacted" in ("IDW 1.0 DE", "IDW Cutover", RIO-RR, "IDW 1.0 Publications", "BAU (Business as Usual)", "SV MoM")) AND status was not in (Accepted, Rejected) on '+cd1 +' AND status was in (Rejected) on ' +cd2
    ...
    print(f"Tickets Rejected between {cd1} - {cd2}: {j}")
