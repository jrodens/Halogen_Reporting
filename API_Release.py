...
datafromjson = jira.search_issues('project = NYC-IO-Surge-Testing and "Epic Link" = NYCTT-1026', maxResults=1000)
    ...
    for link in issue.fields.issuelinks:
        if hasattr(link, "outwardIssue"):
            ...
            if "NYCIO" in outwardIssue.key:
                ...
                s = S(currentIssue)
                d = S(currentStatus)
                epic = issue.fields.customfield_10506
                if epic == "NYCIO-72":
                    epic = 'Securities'
                elif epic == "NYCIO-1963":
                    epic = 'Transaction Gain Loss'
                elif epic == "NYCIO-373":
                    epic = 'Event Notification'
...
newdata.insert(0,"Consumer Ticket", Report)
...
#newdata.insert(6, "Epic", Epic)

#newdata.to_excel('O:/Position Attribute Data.xlsx')

print('Plotting data')
newdata.set_index('Consumer Ticket',inplace = True, drop=True)
...
'''
newdata.to_excel('O:/Consumer Tickets.xlsx')
#newdata= newdata.set_index(['Epic', 'Consumer Ticket'])
newdata = newdata[(newdata.T != 0).any()]
newdata.to_excel('O:/Consumer Tickets.xlsx')
pdf = PdfPages('O:/Consumer Tickets.pdf')
fig = plt.figure()
#fig2 = plt.figure()
#fig3 = plt.figure()
#fig4 = plt.figure()

with PdfPages('O:/Consumer Tickets.pdf') as pdf:
    ax1=fig.add_subplot(111)
    #newdata = newdata.groupby(['Epic']).count()
    ax1=newdata.plot(ax=ax1,kind='bar',stacked=True, figsize=(20,10),color=['#C00000','#002060','#FFC000','#548235','#7030A0'], title="Consumer Tickets")
