...
fig.add_trace(go.Bar(x=q1.iloc[0], y=q1.iloc[1]), name= "Accepted")
fig.add_trace(go.Bar(x=q1.iloc[0], y=q1.iloc[2]), name = "Blocked")
fig.add_trace(go.Bar(x=q1.iloc[0], y=q1.iloc[3]), name = "In Development")
fig.add_trace(go.Bar(x=q1.iloc[0], y=q1.iloc[4]), name = "Not Started")
fig.add_trace(go.Bar(x=q1.iloc[0], y=q1.iloc[5]), name = "QA")
fig.add_trace(go.Bar(x=q1.iloc[0], y=q1.iloc[6]), name="Rejected")
...
with PdfPages(output) as pdf:
    ...
    tempdf=q1.unstack()
    newq1=tempdf.plot(ax=ax1,kind='bar',stacked=True, figsize=(20,10),color=[color_dict.get(x) for x in tempdf.columns], title="Q1 Position Views")

    ax2=fig2.add_subplot(111)
    tempdf=q2.unstack()
    newq2=tempdf.plot(ax=ax2,kind='bar',stacked=True, figsize=(20,10),color=[color_dict.get(x) for x in tempdf.columns], title="Q2 Position Views")

    ax3=fig3.add_subplot(111)
    tempdf=q3.unstack()
    newq3=tempdf.plot(ax=ax3,kind='bar',stacked=True, figsize=(20,10),color=[color_dict.get(x) for x in tempdf.columns], title="Q3 Position Views")

    ax4=fig4.add_subplot(111)
    tempdf=q4.unstack()
    newq4=tempdf.plot(ax=ax4,kind='bar',stacked=True, figsize=(20,10),color=[color_dict.get(x) for x in tempdf.columns], title="Q4 Position Views")
