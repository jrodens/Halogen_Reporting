...
datafromjson = jira.search_issues('project = NYC-RIO-Surge-Halogen-Analysis and "Epic Link" = NYCRSA-1855', maxResults=300)
...
datafromjson2 = requests.get('https://jira.troweprice.com/rest/api/2/search?jql=project%20=%20NYC-RIO-Surge-Halogen%20and%20component%20=%20%22account%20Domain%22&maxResults=1000&fields=status', auth=(username, password)).content
...
output = 'C:\\Users\\' + username + '\\OneDrive - TRowePrice\\AccountAttributes-'+ str(datetime.datetime.today().strftime('%Y%m%d')) +'.pdf'
pdf = PdfPages('O:/Account Reference.pdf')
fig1 = plt.figure()

with PdfPages(output) as pdf:
    ax1=fig1.add_subplot(111)
    newdata=newdata.unstack()
    newdata.plot(ax=ax1,kind='bar',stacked=True, figsize=(20,10),color=[color_dict.get(x) for x in newdata.columns], title="Account Reference")
